...
def add_vxlan(db, vxlan_name, src_ip):
    ...
    vxlan_keys = cfg_db.get_keys('VXLAN_TUNNEL')
    if not vxlan_keys:
        vxlan_count = 0
    else:
...
def del_vxlan(db, vxlan_name):
    ...
    vxlan_keys = cfg_db.get_keys('VXLAN_EVPN_NVO')
    if not vxlan_keys:
        vxlan_count = 0
    else:
        vxlan_count = len(vxlan_keys)
    ...
    vxlan_keys = cfg_db.get_keys("VXLAN_TUNNEL_MAP")
    if not vxlan_keys:
        vxlan_count = 0
    else:
        vxlan_count = len(vxlan_keys)
    ...
    vnet_table = cfg_db.get_table('VNET')
...
def add_vxlan_evpn_nvo(db, nvo_name, vxlan_name):
    ...
    if len(cfg_db.get_entry('VXLAN_TUNNEL', vxlan_name)) == 0:
...
def add_vxlan_map(db, vxlan_name, vlan, vni):
    ...
    if len(cfg_db.get_entry('VLAN', vlan_name)) == 0:
        ctx.fail("{} not configured".format(vlan_name))

    vxlan_table = cfg_db.get_table('VXLAN_TUNNEL_MAP')
...
def del_vxlan_map(db, vxlan_name, vlan, vni):
    ...
    if len(cfg_db.get_entry('VXLAN_TUNNEL', vxlan_name)) == 0:
        ctx.fail("VTEP {} not configured".format(vxlan_name))
    found = 0
    vrf_table = cfg_db.get_table('VRF')
    ...
    cfg_db.set_entry('VXLAN_TUNNEL_MAP', mapname, None)
...
def del_vxlan_map_range(db, vxlan_name, vlan_start, vlan_end, vni_start):
    ...
    for vid in range(vlan_start, vlan_end):
        ...
        vnid = vni_start+vid-vlan_start
        vni_name = '{}'.format(vnid)
        if clicommon.is_vni_vrf_mapped(cfg_db, vni_name) is False:

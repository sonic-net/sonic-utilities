...
def switch_fast_linkup_global_cmd(db, polling_time, guard_time, ber):
    ...
    poll_range = entry.get('FAST_LINKUP_POLLING_TIMER_RANGE').split(',')
    guard_range = entry.get('FAST_LINKUP_GUARD_TIMER_RANGE').split(',')
    ...
    if polling_time is not None:
        if poll_range and not (int(poll_range[0]) <= int(polling_time) <= int(poll_range[1])):
    ...
    if guard_time is not None:
        if guard_range and not (int(guard_range[0]) <= int(guard_time) <= int(guard_range[1])):

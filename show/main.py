...
def igmp_snooping_vlan(vlan_id):
    ...
    optimised_multicast_flood = app_db.get(app_db.APPL_DB, app_l2mc_supress_key, 'ipv4-optimised-multicast-flood')
    if not optimised_multicast_flood:
        optimised_multicast_flood = igmp_snooping_fvs['ipv4-optimised-multicast-flood']
    else :
    ...
    link_local_groups_suppression = app_db.get(app_db.APPL_DB, app_l2mc_supress_key, 'ipv4-link-local-groups-suppression')
    if not link_local_groups_suppression:
        link_local_groups_suppression = igmp_snooping_fvs['ipv4-link-local-groups-suppression']
    else :
...
def igmp_snooping_groups_all():
    ...
    vlan_table_keys = config_db.get_keys('L2MC')
        ...
        for v, s, g in natsorted(member_table_dict):
            if v == vlan_iter:
                ...
                for memb_intf in member_table_dict[(v, s, g)]:
                    memb_intf_string = str(memb_intf) if memb_intf is not None else ""
                    for member_type in member_type_dict[(v, s, g, memb_intf)]:
                        member_type_string = str(member_type) if member_type is not None else ""
...
@cli.group(cls=clicommon.AliasedGroup)
def mld_snooping():
    """Show mld-snooping related information"""
    pass

@mld_snooping.command('vlan')
@click.argument('vlan-id', required=True)
def mld_snooping_vlan(vlan_id):
    ...
    key = "Vlan"+str(vlan_id)
    data = config_db.get_entry('MLD_L2MC', key)
    app_l2mc_supress_key = "L2MC_SUPPRESS_TABLE:Vlan" + str(vlan_id)
    ...
    querier_status = data.get('querier')
    if not querier_status:
        querier_status = mld_snooping_fvs['querier']

    operation_mode = data.get('version')
    if not operation_mode:
        operation_mode = mld_snooping_fvs['version']

    fast_leave = data.get('fast-leave')
    if not fast_leave:
        fast_leave = mld_snooping_fvs['fast-leave']
    ...
    max_response_time = data.get('query-max-response-time')
    if not max_response_time:
        max_response_time = mld_snooping_fvs['query-max-response-time']

    last_member_query_interval = data.get('last-member-query-interval')
    if not last_member_query_interval:
        last_member_query_interval = mld_snooping_fvs['last-member-query-interval']

    query_interval = data.get('query-interval')
    if not query_interval:
        query_interval = mld_snooping_fvs['query-interval']

    optimised_multicast_flood = app_db.get(app_db.APPL_DB, app_l2mc_supress_key, 'ipv6-optimised-multicast-flood')
    if not optimised_multicast_flood:
        optimised_multicast_flood = mld_snooping_fvs['ipv6-optimised-multicast-flood']
    else :
        if optimised_multicast_flood == 'enable':
            optimised_multicast_flood = 'true'
        else:
    ...
    link_local_groups_suppression = app_db.get(app_db.APPL_DB, app_l2mc_supress_key, 'ipv6-link-local-groups-suppression')
    if not link_local_groups_suppression:
        link_local_groups_suppression = mld_snooping_fvs['ipv6-link-local-groups-suppression']
    else :
        if link_local_groups_suppression == 'enable':
            link_local_groups_suppression = 'true'
        else:
    ...
    click.echo("Multicast Router ports: " + app_memb_intf_string)
    click.echo("Querier - " + querier_status)
    click.echo("MLD Operation mode: MLDv" + operation_mode)
    click.echo("Is Fast-Leave Enabled: " + fast_leave_status)
    click.echo("Max Response time = " + max_response_time)
    click.echo("Query Interval = " + query_interval)
    click.echo("Last Member Query Interval = " + last_member_query_interval )
...
@mld_snooping.command('all')
def mld_snooping_vlan():
    """ MLD snooping all"""
    config_db = ConfigDBConnector()
    config_db.connect()
    vlan_dict = {}
    ...
    for vlan_id in natsorted(vlan_dict):
        ...
        if not optimised_multicast_flood:
            optimised_multicast_flood = igmp_snooping_fvs['ipv6-optimised-multicast-flood']
        else :
            if optimised_multicast_flood == 'enable':
                optimised_multicast_flood = 'true'
            else:
        ...
        link_local_groups_suppression = app_db.get(app_db.APPL_DB, app_l2mc_supress_key, 'ipv6-link-local-groups-suppression')
        if not link_local_groups_suppression:
            link_local_groups_suppression = igmp_snooping_fvs['ipv6-link-local-groups-suppression']
        else :
            if link_local_groups_suppression == 'enable':
                link_local_groups_suppression = 'true'
            else:
    ...
    click.echo("Total number of entries:  " + str(len(l2mc_keys))+"\n")
#
# 'groups' group ("show mld_snooping groups")
#
@mld_snooping.group('groups')
@click.pass_context
def mld_snooping_groups(ctx):
    """Show IP MLD snooping information"""
    pass


@mld_snooping_groups.command('all')
def mld_snooping_groups_all():
    """ IGMP snooping groups all """
    config_db = ConfigDBConnector()
    config_db.connect()
    app_db = SonicV2Connector(host='127.0.0.1')
    ...
    vlan_table_keys = config_db.get_keys('MLD_L2MC')
    if not vlan_table_keys:
        return
    ...
    for key in member_table_keys:
        ...
        if len(list_items) == 19:
            ...
            member = list_items[18]
        else :
            continue

        source = str(ipaddress. IPv6Address(source))
        group = str(ipaddress. IPv6Address(group))
        member_type = app_db.get(app_db.APPL_DB, key, 'type')
...
@mld_snooping_groups.command('vlan')
@click.argument('vlan-id', required=True)

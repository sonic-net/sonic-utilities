...
class GrubBootloader(OnieInstallerBootloader):
    ...
    def is_secure_upgrade_image_verification_supported(self):
        ...
        #Disabling the check for cisco / cisco-8000 platforms as platform-side support is not ready yet. This will be removed once platform
        #support is added.
        ASIC_TYPE=$(sonic-cfggen -y /etc/sonic/sonic_version.yml -v asic_type)
        if [ -d "/sys/firmware/efi/efivars" ] && [[ ${ASIC_TYPE} != *"cisco-8000"* && ${ASIC_TYPE} != "cisco" ]]; then

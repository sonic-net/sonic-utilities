...
# Import show and config first (like BFD test does)
# This will use the default namespace list (empty for single-ASIC)
...
class TestVlanMultiAsic(object):
    ...
    def test_config_vlan_add_member_except_flag_with_namespace(self):
        ...
        result = runner.invoke(config.config.commands["vlan"],
                              ["-n", "asic0", "member", "add", "1000,4000", "Ethernet16", "--multiple", "--except_flag"],
                              obj=db)
        print(result.exit_code)
        print(result.output)

...
class TestConfigReloadMasic(object):
    ...
    def test_config_reload_multiple_files_no_file_exception(self):
        dummy_cfg_file = os.path.join(os.sep, "tmp", "config.json")
        dummy_cfg_file_asic0 = os.path.join(os.sep, "tmp", "config0.json")
        dummy_cfg_file_asic1 = os.path.join(os.sep, "tmp", "config1.json")
        self._create_dummy_config(dummy_cfg_file, {})
        self._create_dummy_config(dummy_cfg_file_asic0, {})
        self._create_dummy_config(dummy_cfg_file_asic1, {})

        with mock.patch("utilities_common.cli.run_command",
                        mock.MagicMock(side_effect=mock_run_command_side_effect)):
            runner = CliRunner()
            # add unnecessary spaces to cause an exception
            cfg_files = f"{dummy_cfg_file},{dummy_cfg_file_asic0},  {dummy_cfg_file_asic1}"
            ...
            assert result.exit_code != 0
            assert "[Errno 2] No such file or directory: '  /tmp/config1.json'" in str(result.exception)

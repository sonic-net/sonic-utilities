...
def test_get_reboot_status_accepts_inactive_without_status(tmp_path):
    script = f'''
EXIT_SUCCESS=0
EXIT_ERROR=1
source "{SCRIPT}"
timeout() {{ shift; docker "$@"; }}
docker() {{ printf '{{"active":false}}\n'; }}
get_reboot_status 169.254.200.1 50052
'''
    result = subprocess.run(["bash", "-c", script], capture_output=True, text=True)
    assert result.returncode == 0
...
def test_get_reboot_status_rejects_unknown_status(tmp_path):
    script = f'''
EXIT_SUCCESS=0
EXIT_ERROR=1
source "{SCRIPT}"
timeout() {{ shift; docker "$@"; }}
docker() {{ printf '{{"active":false,"status":{{"status":0}}}}\n'; }}
get_reboot_status 169.254.200.1 50052
'''
    result = subprocess.run(["bash", "-c", script], capture_output=True, text=True)
    assert result.returncode != 0
...
def test_reboot_dpu_restores_after_partial_pre_shutdown_failure(tmp_path):
    platform_json = tmp_path / "platform.json"
    platform_json.write_text('{"DPUS":{"dpu0":{"bus_info":"0000:00:00.0"}}}')
    marker = tmp_path / "post-startup"
    script = f'''
EXIT_SUCCESS=0
EXIT_ERROR=1
PLATFORM_JSON_PATH="{platform_json}"
...
clear_module_state_transition_flag() {{ return 0; }}
gnmi_reboot_dpu() {{ return 0; }}
module_pre_shutdown() {{ return 1; }}
module_post_startup() {{ touch "{marker}"; return 0; }}
reboot_dpu dpu0 DPU
'''
    result = subprocess.run(["bash", "-c", script], capture_output=True, text=True)
    assert result.returncode != 0

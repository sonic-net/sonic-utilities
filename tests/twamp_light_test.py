...
Name              Status    Sender IP:PORT    Reflector IP:PORT      Packet Count  Monitor Time      Tx Interval    Stats Interval    Timeout
----------------  --------  ----------------  -------------------  --------------  --------------  -------------  ----------------  ---------
SESSION_SEND_VM1  active    10.20.100.1:862   10.10.100.2:863                 100  -                         100             15000          5
...
----------------  --------  -----------------  -------------------  --------------  --------------  -------------  ----------------  ---------
SESSION_SEND_VM1  active    10.20.100.1:862    10.10.100.2:863                 100  -                         100             15000          5
SESSION_SEND_VM2  active    192.168.50.3:1862  192.168.100.2:1863             2000  -                         200              1500          5
...
class TestTwampLightshow():
    ...
    def test_show_twamp_light_stats_twoway_delay(self):
        ...
        result = self.runner.invoke(show.cli.commands["twamp-light"].commands["statistics"], ["twoway-delay", "--lastest_number", "3"])
        print(result.exit_code)
        print(result.output)
        assert result.exit_code == 0
    ...
    def test_show_twamp_light_stats_twoway_loss(self):
        ...
        result = self.runner.invoke(show.cli.commands["twamp-light"].commands["statistics"], ["twoway-loss", "SESSION_SEND_VM1", "-b", ])
        print(result.exit_code)
        print(result.output)
        assert result.exit_code == 0
    ...
    def test_config_add_reflector_normal(self):
        ...
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-reflector"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_NO_PORT, TL_DST_IP_PORT], obj=db)
        assert result.output == twamp_light_null_output
        assert result.exit_code == 0, "twamp ligith config reflector normal config test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_remove_reflector_normal(self):
        ...
        # config valid twamp light reflector sender port
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-reflector"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_NO_PORT, TL_DST_IP_PORT, "--vrf", "VRF_TEST", "--dscp", "10", "--ttl", "20", "--timestamp-format", "ntp"], obj=db)
        assert result.output == twamp_light_null_output
        assert result.exit_code == 0, "twamp ligith config reflector normal config test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
        ...
        with patch("config.twamp_light.check_if_twamp_session_exist") as mock_function:
            ...
            result = self.runner.invoke(config.config.commands["twamp-light"].commands["remove"], [TL_SESSION_NAME], obj=db)
            assert result.output == twamp_light_null_output
            assert result.exit_code == 0, "twamp ligith config reflector normal config test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
            twamp_session = db.cfgdb.get_entry(TL_CFGDB_KEY_PREFIX, TL_SESSION_NAME)
    ...
    def test_add_reflector_with_invalid_send_port(self):
        ...
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-reflector"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_PORT_INVLD, TL_DST_IP_PORT], obj=db)
        print(result.output)
        assert result.output == config_twamp_light_reflector_with_invalid_src_port
        assert result.exit_code != 0, "reflector invalid src port test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)

        # config invalid twamp light reflector sender port
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-reflector"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_PORT, TL_DST_IP_PORT_INVLD], obj=db)
        print(result.output)
    ...
    def test_config_add_sender_continuous_normal(self):
        ...
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["continuous"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_PORT, TL_DST_IP_PORT, TL_MONITOR_TIME, TL_TX_INTERVAL, TL_TIMEOUT, TL_STATS_INTERVAL,  "--vrf",  "VRF_TEST", "--dscp", "15", "--ttl", "56", "--timestamp-format", "ptp"], obj=db)
        assert result.output == twamp_light_null_output
        assert result.exit_code == 0, "twamp ligith config reflector normal config test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_add_sender_continuous_with_invalid_stats_interval_monitor_time_none_zero(self):
        ...
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["continuous"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_PORT, TL_DST_IP_PORT, TL_MONITOR_TIME, TL_TX_INTERVAL, TL_TIMEOUT], obj=db)
        assert result.output == twamp_light_null_output
        assert result.exit_code == 0, "twamp ligith config reflector normal config test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_add_sender_continuous_with_invalid_stats_interval_monitor_time_zero(self):
        ...
        # config valid twamp light sender with continuous mode， without config stats internval
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["continuous"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_PORT, TL_DST_IP_PORT, "0", TL_TX_INTERVAL, TL_TIMEOUT], obj=db)
        assert result.output == twamp_light_add_continous_sender_with_invalid_monitor_time_output
        assert result.exit_code != 0, "twamp ligith config continuous sender with invalid monitor time. test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_add_sender_continuous_with_invalid_stats_interval_too_small(self):
        ...
        # config valid twamp light sender with continuous mode， without config stats internval
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["continuous"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_PORT, TL_DST_IP_PORT, TL_MONITOR_TIME, TL_TX_INTERVAL, TL_TIMEOUT, "2000"], obj=db)
        assert result.output == twamp_light_add_continous_sender_with_invalid_stats_interval
        assert result.exit_code != 0, "twamp ligith config continuous sender with invalid stats intervald. test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_add_sender_pkt_count_normal(self):
        ...
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["packet-count"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_PORT, TL_DST_IP_NO_PORT, TL_PKT_CNT, TL_TX_INTERVAL, TL_TIMEOUT, TL_STATS_INTERVAL, "--vrf",  "VRF_TEST", "--dscp", "15", "--ttl", "56", "--timestamp-format", "ptp"], obj=db)
        assert result.output == twamp_light_null_output
        assert result.exit_code == 0, "twamp ligith config sender normal config test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_add_sender_pkt_count_with_invalid_stats_interval(self):
        ...
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["packet-count"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_PORT, TL_DST_IP_NO_PORT, TL_PKT_CNT, TL_TX_INTERVAL, TL_TIMEOUT], obj=db)
        assert result.output == twamp_light_null_output
        assert result.exit_code == 0, "twamp ligith config sender normal config test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_add_sender_while_session_exist(self):
        ...
        with patch("config.twamp_light.check_if_twamp_session_exist") as mock_function:
            ...
            result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["packet-count"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_DFT_PORT, TL_DST_IP_DFT_PORT, TL_PKT_CNT, TL_TX_INTERVAL, TL_TIMEOUT, TL_STATS_INTERVAL], obj=db)
            assert result.output == config_twamp_light_while_session_exist_error_info
            print(result.output)
            assert result.exit_code != 0, "twamp ligith config sender while session is exist, should report error info. test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_add_sender_start_with_no_session(self):
        ...
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["start"], [TL_SESSION_NAME], obj=db)
        assert result.output == twamp_light_start_session_without_session_output
        assert result.exit_code != 0, "twamp ligith config sender start session test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)

    #@patch("config.twamp_light.ConfigDBConnector.get_table", mock.Mock(return_value={"TWAMP_SESSION_TABLE|session_name1"}))
    ...
    def test_config_add_sender_start_single_session(self):
        ...
        with patch("config.twamp_light.check_if_twamp_session_exist") as mock_function:
            ...
            result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["packet-count"].commands["add"], [TL_SESSION_NAME, TL_SRC_IP_DFT_PORT, TL_DST_IP_DFT_PORT, TL_PKT_CNT, TL_TX_INTERVAL, TL_TIMEOUT, TL_STATS_INTERVAL], obj=db)
            assert result.output == twamp_light_null_output
            assert result.exit_code == 0, "twamp ligith config sender with default port config test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)
    ...
    def test_config_add_sender_start_all_session(self):
        ...
        # Start session
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["start"], ["all"], obj=db)
        assert result.output == twamp_light_null_output
        assert result.exit_code == 0, "twamp ligith config sender start session test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)

        # Stop session
        result = self.runner.invoke(config.config.commands["twamp-light"].commands["session-sender"].commands["stop"], ["all"], obj=db)
        assert result.output == twamp_light_null_output
        assert result.exit_code == 0, "twamp ligith config sender start session test case with code {}:{} Output:{}".format(type(result.exit_code), result.exit_code, result.output)

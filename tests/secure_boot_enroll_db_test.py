...
def _run_enrollment(tmp_path, authorized, append_signed=True, check=False):
...
case "$1" in
    ...
    cms)
        ...
        [ "$DB_AUTH_AUTHORIZED" = "yes" ] &&
            [ "$DB_AUTH_APPEND_SIGNED" = "yes" ] &&
            [ -s "$content" ]
        ;;
    *) exit 1 ;;
esac
    ...
    env.update(
        {
            ...
            "DB_AUTH_APPEND_SIGNED": "yes" if append_signed else "no",
...
def test_rejects_unauthorized_db_auth_before_firmware_write(tmp_path):
    ...
    assert (
        ...
        "DB.auth is not a valid append update authorized by an enrolled PK or KEK"
    ) in result.stdout
    assert not update_marker.exists()
    assert not auth_dir.exists()


def test_rejects_replace_only_db_auth_before_firmware_write(tmp_path):
    result, auth_dir, update_marker, _ = _run_enrollment(
        tmp_path, authorized=True, append_signed=False
    )

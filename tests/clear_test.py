...
class TestClearArp(object):
    ...
    @patch('clear.main.run_command', MagicMock(return_value=('192.168.1.1 dev Ethernet0 lladdr 00:11:22:33:44:55 REACHABLE', '')))
    @patch('clear.main.multi_asic.is_multi_asic', MagicMock(return_value=False))
    ...
    def test_clear_arp_multi_asic_with_namespace(self, mock_run_command):
        ...
        assert result.exit_code == 0
        mock_run_command.assert_called_with(['sudo', 'ip', 'netns', 'exec', 'asic0', 'ip', '-4', '-s', '-s', 'neigh', 'flush', 'all'])
...
class TestClearNdp(object):
    ...
    @patch('clear.main.run_command', MagicMock(return_value=('fe80::1 dev Ethernet0 lladdr 00:11:22:33:44:55 REACHABLE', '')))
    @patch('clear.main.multi_asic.is_multi_asic', MagicMock(return_value=False))
    ...
    def test_clear_ndp_multi_asic_with_namespace(self, mock_run_command):
        ...
        assert result.exit_code == 0
        mock_run_command.assert_called_with(['sudo', 'ip', 'netns', 'exec', 'asic0', 'ip', '-6', '-s', '-s', 'neigh', 'flush', 'all'])

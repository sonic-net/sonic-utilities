...
class TestSonicKdumpConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_geteuid = patch('os.geteuid', return_value=0)
        self.mock_geteuid = self.patcher_geteuid.start()

    def tearDown(self):
        self.patcher_exit.stop()
        self.patcher_geteuid.stop()

    @patch('kdump_config.get_current_image', return_value='image')
    @patch('kdump_config.cmd_kdump_enable', return_value=True)
    def test_enable(self, mock_enable, mock_image):
        sys.argv = ['kdump-config', '--enable']
        kdump_config.main()
        mock_enable.assert_called_once()
        mock_image.assert_called_once()

    @patch('kdump_config.cmd_kdump_config_next', return_value=True)
    def test_config_next(self, mock_config_next):
        sys.argv = ['kdump-config', '--config-next']
        kdump_config.main()
        mock_config_next.assert_called_once()

    @patch('kdump_config.cmd_kdump_disable', return_value=True)
    def test_disable(self, mock_disable):
        sys.argv = ['kdump-config', '--disable']
        kdump_config.main()
        mock_disable.assert_called_once()

    @patch('kdump_config.cmd_kdump_memory')
    def test_memory(self, mock_memory):
        sys.argv = ['kdump-config', '--memory', '256M']
        kdump_config.main()
        mock_memory.assert_called_once_with(False, '256M')

    @patch('kdump_config.cmd_kdump_remote')
    def test_remote(self, mock_remote):
        sys.argv = ['kdump-config', '--remote']
        kdump_config.main()
        mock_remote.assert_called_once_with(False)

    @patch('kdump_config.cmd_kdump_ssh_string')
    def test_ssh_string(self, mock_ssh):
        sys.argv = ['kdump-config', '--ssh_string', 'user@host']
        kdump_config.main()
        mock_ssh.assert_called_once_with(False, 'user@host')

    @patch('kdump_config.cmd_kdump_ssh_path')
    def test_ssh_path(self, mock_ssh_path):
        sys.argv = ['kdump-config', '--ssh_path', '/remote/path']
        kdump_config.main()
        mock_ssh_path.assert_called_once_with(False, '/remote/path')

    @patch('kdump_config.cmd_kdump_num_dumps')
    def test_num_dumps(self, mock_num_dumps):
        sys.argv = ['kdump-config', '--num_dumps']
        kdump_config.main()
        mock_num_dumps.assert_called_once_with(False, True)

    @patch('kdump_config.cmd_dump_db')
    def test_dump_db(self, mock_db):
        sys.argv = ['kdump-config', '--dump-db']
        kdump_config.main()
        mock_db.assert_called_once()

    @patch('kdump_config.cmd_dump_status_json')
    def test_status_json(self, mock_status):
        sys.argv = ['kdump-config', '--status-json']
        kdump_config.main()
        mock_status.assert_called_once()

    @patch('kdump_config.cmd_dump_config_json')
    def test_config_json(self, mock_config_json):
        sys.argv = ['kdump-config', '--config-json']
        kdump_config.main()
        mock_config_json.assert_called_once()

    @patch('kdump_config.cmd_dump_kdump_records_json')
    def test_kdump_records_json(self, mock_kdump_records):
        sys.argv = ['kdump-config', '--kdump-records-json']
        kdump_config.main()
        mock_kdump_records.assert_called_once()

    def test_no_arguments(self):
        sys.argv = ['kdump-config']
        with patch('argparse.ArgumentParser.print_help') as mock_help:
            kdump_config.main()
            mock_help.assert_called_once()
            self.mock_exit.assert_called_with(1)

    @patch('kdump_config.cmd_kdump_enable', side_effect=Exception("Boom"))
    @patch('kdump_config.get_current_image', return_value='img')
    @patch('kdump_config.print_err')
    def test_exception_handling(self, mock_printerr, mock_image, mock_enable):
        sys.argv = ['kdump-config', '--enable']
        kdump_config.main()
        mock_printerr.assert_called_once()
        self.mock_exit.assert_called_with(1)

    def test_root_check(self):
        self.mock_geteuid.return_value = 1000  # not root
        sys.argv = ['kdump-config', '--enable']
        kdump_config.main()
        self.mock_exit.assert_called_with("Root privileges required for this operation")

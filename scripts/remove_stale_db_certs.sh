...
#      active ESP -- in particular the firmware fallback loader, shim, GRUB,
#      MokManager, and every remaining installed image's kernel.
...
#   4. SAFETY GATE: refuse to proceed unless every boot-chain binary keeps at
#      least one db signer that we can reinstall. This guarantees the db
#      certificate signing the fallback loader, shim, GRUB, MokManager, or any
#      remaining installed kernel is not lost.
...
#
# It is bundled into the SONiC image and can be invoked by an installer flow
# after obsolete images are removed, or run standalone. Callers can first use
# --plan to inspect the machine-readable "PRUNE_PLAN ..." summary, then use
# --commit after resolving any missing boot-critical signer authorization.
#
# Requirements
...
Usage: $PROG [options]
  ...
  --remove-all FILE  Signed .auth that empties db (deletes all certs)
      ...
      --allow-essential-missing
                     Proceed with --commit even if a boot-chain binary would
                     lose every db signer because no reinstall .auth is
                     available, without asking. DANGEROUS: the system may fail
...
for tool in efi-readvar sig-list-to-certs sbverify openssl efi-updatevar python3; do
...
discover_binaries() {
    _d="$1"
    [ -d "$_d" ] || return 0
    for _pat in 'BOOT*.EFI' 'boot*.efi' 'shim*.efi' 'mm*.efi' 'grub*.efi' \
                'fb*.efi' 'fallback*.efi' 'vmlinuz-*'; do
...
# A remove-all update must end immediately after its authentication header.
# Any remaining bytes are an ESL payload and would append or replace db entries.
auth_has_empty_payload() {
    _auth="$1"
    [ -s "$_auth" ] || return 1
    _dw="$(read_u32le "$_auth" 16)" || return 1
    { [ -n "$_dw" ] && [ "$_dw" -ge 24 ]; } || return 1
    _size="$(wc -c < "$_auth" | tr -d '[:space:]')"
    [ "$_size" -eq $((16 + _dw)) ]
}

# Verify an authenticated db update against the exact content and attributes
# that efi-updatevar will submit to firmware.
verify_db_auth() {
    _auth="$1"; _attributes="$2"; _require_payload="$3"; _prefix="$4"
    _pkcs7="$_prefix.p7b"
    _content="$_prefix.content"

    python3 - "$_auth" "$_pkcs7" "$_content" "$_attributes" "$_require_payload" <<'PY'
import struct
import sys
...
if int(sys.argv[5]) and not payload:
...
signed_content = (
    "db".encode("utf-16-le")
    + bytes.fromhex("cbb219d73a3d9645a3bcdad00e67656f")
    + struct.pack("<I", int(sys.argv[4], 0))
    + auth[:16]
    + payload
)
...
PY
    ...
    openssl cms -verify -binary -inform DER -in "$_pkcs7" \
        -content "$_content" -no_attr_verify -no_check_time \
...
# Every remaining installed image is boot-critical, including its kernel.
if [ -n "$IMAGES" ]; then
...
# Include every ESP loader directory as well. EFI/BOOT is the firmware fallback
# path and is therefore boot-critical; other directories are used only to retain
# certificates that still sign an EFI binary.
for d in "$ESP_PATH"/*/; do
    [ -d "$d" ] || continue
    esp_name="$(basename "$d")"
    esp_critical=0
    case "$esp_name" in
        BOOT|boot) esp_critical=1 ;;
    esac
    printf '%s|%s|%s|%s\n' "ESP:$esp_name" "active" "${d%/}" "$esp_critical" >> "$TARGETS"
done
...
# ---------------------------------------------------------------------------
# SAFETY GATE: every boot-chain binary must keep a reinstallable db signer
# ---------------------------------------------------------------------------
CRIT_FAIL=0
CRIT_AT_RISK=0
CRIT_TOTAL=0
if [ -s "$CRIT_BINS" ]; then
    while IFS= read -r cbin; do
        ...
        if [ -z "$matched" ]; then
            warn "boot-chain binary has NO db signer at all: $cbin"
        ...
        if [ "$keep" -eq 0 ]; then
            echo "$PROG: SAFETY: boot-chain binary would lose all db signers: $cbin" >&2
            for idx in $matched; do
    ...
    warn "no boot-chain binaries were identified"
fi
...
#   essential_missing = boot-chain binaries that would lose every db signer
...
if [ "$COMMIT" -ne 1 ]; then
    # Dry-run: report an essential-missing risk but never prompt or fail on it here.
    if [ "$CRIT_FAIL" -ne 0 ]; then
        warn "$CRIT_AT_RISK boot-chain binaries would lose every db signer (essential db certificate missing); --commit would ask for confirmation"
    fi
...
# ---------------------------------------------------------------------------
# Committing. This is non-interactive, with a single exception: if an essential
# boot-chain db certificate would be lost, that may
# leave the device unable to boot under Secure Boot, so ask before proceeding.
# ---------------------------------------------------------------------------
if [ "$CRIT_FAIL" -ne 0 ]; then
    if [ "$ALLOW_ESSENTIAL_MISSING" -eq 1 ]; then
        warn "proceeding despite $CRIT_AT_RISK boot-chain binaries without a reinstall .auth (--allow-essential-missing); the system may fail Secure Boot verification after reboot"
    elif [ -t 0 ]; then
        echo "WARNING: $CRIT_AT_RISK boot-chain binaries would lose every UEFI db signer." >&2
        echo "         No reinstall certificate is available for them, so this device may fail to boot under" >&2
        echo "         Secure Boot after the prune. Provide the missing .auth in $AUTH_DIR to avoid this." >&2
        printf 'Proceed anyway and risk an unbootable device? Type "yes" to continue: '
        read -r _ans
        [ "$_ans" = "yes" ] || die "aborted: essential db certificate missing"
        # Operator approved the risk; treat the rest of the run as an approved essential loss
        # so the post-apply check warns instead of failing.
        ALLOW_ESSENTIAL_MISSING=1
    else
        die "a boot-chain binary would lose every db signer and no TTY is available to confirm. Refusing to touch db. Provide the missing .auth in $AUTH_DIR (or pass --allow-essential-missing to override)."
    fi
fi
...
auth_has_empty_payload "$REMOVE_ALL_AUTH" \
    || die "remove-all .auth must contain an empty signature-list payload: $REMOVE_ALL_AUTH"

# Build the firmware authority store used to validate every update before db is
# touched. This prevents a replace-only or unauthorized reinstall .auth from
# leaving db partially restored after remove-all succeeds.
TRUSTED_CERTS="$WORK/firmware-authorities.pem"
: > "$TRUSTED_CERTS"
for variable in PK KEK; do
    authority_esl="$WORK/$variable.esl"
    authority_prefix="$WORK/$variable"
    efi-readvar -v "$variable" -o "$authority_esl" >/dev/null 2>&1 \
        || die "failed to read enrolled UEFI $variable certificates"
    sig-list-to-certs "$authority_esl" "$authority_prefix" >/dev/null 2>&1 \
        || die "failed to parse enrolled UEFI $variable certificates"
    for certificate in "$authority_prefix"-*.der; do
        [ -e "$certificate" ] || continue
        openssl x509 -inform DER -in "$certificate" >> "$TRUSTED_CERTS" \
            || die "failed to convert an enrolled UEFI $variable certificate"
    done
done
[ -s "$TRUSTED_CERTS" ] || die "no enrolled PK or KEK certificates are available"

verify_db_auth "$REMOVE_ALL_AUTH" 0x27 0 "$WORK/remove-all" \
    || die "remove-all .auth is not an authorized db replacement update: $REMOVE_ALL_AUTH"

# Verify each selected reinstall .auth exists and is authorized specifically
# for append semantics before applying remove-all.
_auth_index=0
while IFS= read -r a; do
    [ -n "$a" ] || continue
    [ -s "$a" ] || die "selected reinstall .auth is missing or empty: $a"
    verify_db_auth "$a" 0x67 1 "$WORK/reinstall-$_auth_index" \
        || die "selected reinstall .auth is not an authorized db append update: $a"
    _auth_index=$((_auth_index + 1))
done < "$SEL_AUTHS"
...
# Re-verify each boot-chain binary against the new db.
CRIT_POST_FAIL=0
...
if [ "$CRIT_POST_FAIL" -ne 0 ]; then
    if [ "$ALLOW_ESSENTIAL_MISSING" -eq 1 ]; then
        warn "CRITICAL: a boot-chain binary no longer verifies against db (approved via --allow-essential-missing). DO NOT REBOOT with Secure Boot enforcing until a valid image-signing db certificate is enrolled."
    else
        die "CRITICAL: a boot-chain binary no longer verifies against db. DO NOT REBOOT with Secure Boot enforcing. Re-enroll a valid image-signing db certificate before rebooting."
    fi
fi
if [ "$_fail" -ne 0 ]; then
...
echo
if [ "$CRIT_POST_FAIL" -ne 0 ]; then
    echo "Done: db reset to $NEW_COUNT image-signing certificate(s); WARNING: $CRIT_POST_FAIL_COUNT boot-chain binaries do NOT verify against the new db (see above)."
else
    echo "Done: db reset to $NEW_COUNT image-signing certificate(s); all boot-chain binaries verify."
fi
exit 0

...
if [ "$COMMIT" -ne 1 ]; then
    ...
    if [ "$CRIT_FAIL" -ne 0 ]; then
        warn "$CRIT_AT_RISK boot-chain binary/ies would lose every db signer (essential db certificate missing); --commit would ask for confirmation"
    fi
...
if [ "$CRIT_FAIL" -ne 0 ]; then
    if [ "$ALLOW_ESSENTIAL_MISSING" -eq 1 ]; then
        warn "proceeding despite $CRIT_AT_RISK boot-chain binary/ies without a reinstall .auth (--allow-essential-missing); the system may fail Secure Boot verification after reboot"
    elif [ -t 0 ]; then
        echo "WARNING: $CRIT_AT_RISK boot-chain binary/ies would lose every UEFI db signer." >&2
...
echo
if [ "$CRIT_POST_FAIL" -ne 0 ]; then
    echo "Done: db reset to $NEW_COUNT image-signing certificate(s); WARNING: $CRIT_POST_FAIL_COUNT boot-chain binary/ies do NOT verify against the new db (see above)."
else

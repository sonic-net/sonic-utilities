...
Usage: $PROG [options]
  ...
  --remove-all FILE  Signed .auth that empties db (deletes all certs)
      ...
      --allow-essential-missing
                     ...
                     reinstall .auth is available, without asking. DANGEROUS: the
                     system may fail Secure Boot verification after reboot. Off by default.
...
if [ "$COMMIT" -ne 1 ]; then
    ...
    if [ "$CRIT_FAIL" -ne 0 ]; then
        warn "$CRIT_AT_RISK boot-critical binary/ies would lose every db signer (essential db certificate missing); --commit would ask for confirmation"
    fi
...
# (boot-critical shim/grub/MokManager) db certificate would be lost, that may
...
if [ "$CRIT_FAIL" -ne 0 ]; then
    if [ "$ALLOW_ESSENTIAL_MISSING" -eq 1 ]; then
        ...
        echo "WARNING: $CRIT_AT_RISK boot-critical (shim/grub/MokManager) binary/ies would lose every UEFI db signer." >&2
        ...
        die "a boot-critical (shim/grub/MokManager) binary would lose every db signer and no TTY is available to confirm. Refusing to touch db. Provide the missing .auth in $AUTH_DIR (or pass --allow-essential-missing to override)."
    fi
fi

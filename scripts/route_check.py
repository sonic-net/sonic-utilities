...
def fetch_routes(ipv6=False, namespace=multi_asic.DEFAULT_NAMESPACE):
    ...
    Returns missing routes as {'prefix', 'protocol'} dicts for compatibility
    with mitigate_installed_not_offloaded_frr_routes().  Text output only
    surfaces BGP routes (matched by the leading 'B'), so protocol is 'bgp'.
    """
    missing_routes = []
    failing_routes = []
    ...
    try:
        with subprocess.Popen(cmd, stdout=subprocess.PIPE,
            ...
            for line in proc.stdout:
                ...
                prefix = m.group(2)
                if flag == 'q':
                    missing_routes.append({'prefix': prefix, 'protocol': 'bgp'})
                if flag == 'o':

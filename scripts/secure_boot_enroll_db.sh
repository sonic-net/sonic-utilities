...
# ContentInfo wrapper.
if ! python3 - "$DB_AUTH" "$AUTH_PKCS7" <<'PY'
import sys
...
# The update content is detached and firmware-specific. Disable content and
# attribute verification here, but still require OpenSSL to validate the actual
# CMS signer certificate against the enrolled authority set.
if ! openssl cms -verify -inform DER -in "$AUTH_PKCS7" \
        -content /dev/null -no_content_verify -no_attr_verify -no_check_time \
        -purpose any -partial_chain -CAfile "$TRUSTED_CERTS" \
        -out /dev/null >/dev/null 2>&1; then
    log "ERROR: failed to enroll db certificate: DB.auth is not authorized by an enrolled PK or KEK"
    exit 1
fi
